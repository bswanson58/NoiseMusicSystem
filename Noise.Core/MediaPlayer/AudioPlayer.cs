			   ...
			   ( mEqEnabled )) {
				try {
					...
					foreach( var band in mEq.Bands ) {
						stream.EqChannels.Add( band.BandId, Bass.BASS_ChannelSetFX( stream.Channel, BASSFXType.BASS_FX_DX8_PARAMEQ, 0 ));
					}
					...
					foreach( var band in mEq.Bands ) {
						...
						Bass.BASS_FXSetParameters( stream.EqChannels[band.BandId], eq );
					}
				}
				catch( Exception ex ) {
