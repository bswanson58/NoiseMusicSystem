...
namespace Noise.Core.DataExchange {
	internal class ImportFavorites : IDataImport {
		...
		public ImportFavorites( IDataProvider dataProvider ) {
					...
					if(!string.IsNullOrWhiteSpace( artist )) {
						...
						if( dbArtist != null ) {
							...
							if(!string.IsNullOrEmpty( album )) {
								using( var albumList = mDataProvider.GetAlbumList( dbArtist )) {
									...
									if( dbAlbum != null ) {
										...
										if(!string.IsNullOrWhiteSpace( track )) {
											using( var trackList = mDataProvider.GetTrackList( dbAlbum )) {
